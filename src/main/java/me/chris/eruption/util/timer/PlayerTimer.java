package me.chris.eruption.util.timer;
...
import me.chris.eruption.util.timer.event.TimerClearEvent;
import me.chris.eruption.util.timer.event.TimerExtendEvent;
import me.chris.eruption.util.timer.event.TimerPauseEvent;
import me.chris.eruption.util.timer.event.TimerStartEvent;
...
public abstract class PlayerTimer extends Timer {
	...
	public TimerCooldown clearCooldown(Player player, UUID playerUUID) {
		...
		if (runnable != null) {
			runnable.cancel();
			if (player == null) {
				EruptionPlugin.getInstance().getServer().getPluginManager().callEvent(new TimerClearEvent(playerUUID, this));
			} else {
				EruptionPlugin.getInstance().getServer().getPluginManager().callEvent(new TimerClearEvent(player, this));
			}
		}
	...
	public boolean setCooldown(Player player, UUID playerUUID, long duration, boolean overwrite,
		...
		if (runnable != null) {
			...
			EruptionPlugin.getInstance().getServer().getPluginManager().callEvent(event);
			if (event.isCancelled()) {
				return false;
			}
			...
			EruptionPlugin.getInstance().getServer().getPluginManager().callEvent(new TimerStartEvent(player, playerUUID, this, duration));
