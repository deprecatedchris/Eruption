...
import me.chris.eruption.util.timer.event.TimerExpireEvent;
				...
				public void run() {
					if (TimerCooldown.this.timer instanceof PlayerTimer && owner != null) {
						((PlayerTimer) timer).handleExpiry(
								EruptionPlugin.getInstance().getServer().getPlayer(TimerCooldown.this.owner), TimerCooldown.this.owner);
					}

					EruptionPlugin.getInstance().getServer().getPluginManager().callEvent(
